...
Game.prototype = {
  ...
  setupControls: function() {
    ...
    var transformControls = new THREE.TransformControls( engine.currentCamera, document.body )
    engine.helpersScene.add( transformControls )
    ...
    function setTransformControlTarget() {
      ...
      if ( this.currentMode === 'scene' && target ) {
        ...
        // directionVector = transformControls.position.clone().sub(lookTarget).setY(0).normalize()
        // var angle = 0.75 * Math.PI + Math.atan2(directionVector.x,directionVector.z)
        // transformControls.setRotationFromAxisAngle( new THREE.Vector3( 0, 1, 0 ), angle )
      }
